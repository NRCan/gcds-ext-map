...
| `search`     | The `search` link relation is used with the `tref="..."` attribute to define a URL template for a search endpoint. The template must contain the `{searchTerms}` variable reference, which is replaced with the user's URL-encoded search query. The search is triggered when the user presses Enter or clicks a suggestion. The response is expected to be a GeoJSON `FeatureCollection` (the default handler format). Only the first `<map-link rel="search">` per `<map-layer>` is honored. The link must be a direct child of `<map-layer>` (for local/inline layers) or placed inside `<map-head>` (for remote `.mapml` layers). The search control is opt-in via `controlslist="search"` on the `<gcds-ext-map>` element. The search button is disabled when no visible (checked) layer provides a `<map-link rel="search">`. |
...
Projection values [defined by the polyfill](../gcds-ext-map#projection) include: 
...
```html
<gcds-ext-map  projection="OSMTILE" zoom="1" lat="0" lon="0" controls>
 <map-layer label="OpenStreetMap" src="node_modules/@gcds-extensions/map/dist/gcds-ext-map/assets/mapml/en/osmtile/osm" checked hidden  ></map-layer>
...
The search control is opt-in: add `controlslist="search"` to your `<gcds-ext-map>` 
element. A magnifying-glass button appears in the 
...
```html
<gcds-ext-map projection="OSMTILE" zoom="14" lat="45.4" lon="-75.7"
...
```html
<gcds-ext-map projection="CBMTILE" zoom="5" lat="45.4" lon="-75.7"
              controls controlslist="search">
...
The polyfill dispatches two cancelable `CustomEvent`s on the `<gcds-ext-map>` 
element:
