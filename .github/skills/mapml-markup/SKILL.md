...
Often there is a need to generate markup for what are always empty elements e.g. `<map-link>`, `<map-meta>` or `<map-input>`.  Do NOT create self-closing tags for such elements - use a full `</end-tag>` form with no 
whitespace in the element body.  This avoids parsing errors when such content is copy-pasted by a user (or by
you, the agent) from the XHTML/XML document into the body of an HTML document, because HTML documents are parsed by the HTML parser, which has different markup semantics.