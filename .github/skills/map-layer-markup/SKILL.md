...
All `<gcds-ext-map>` content is contained by one or more `<map-layer>` elements, either inline or by remote fetch representing the layer's content.
...
Map media queries can include map properties as documented in the [matchMedia](../../api/gcds-ext-map-api#matchmedia) API.
...
```html
<gcds-ext-map projection="CBMTILE" zoom="2" lat="45" lon="-90" controls>
      <map-layer opacity = "0.5" label="CBMT" src="node_modules/@gcds-extensions/map/dist/gcds-ext-map/assets/mapml/en/cbmtile/cbmt" checked></map-layer>
</gcds-ext-map>
```
