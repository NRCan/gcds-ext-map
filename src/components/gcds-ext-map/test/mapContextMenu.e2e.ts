...
test.describe('Playwright Map Context Menu Tests', () => {
  ...
  test.beforeAll(async () => {
    ...
    await page.goto('/test/gcds-ext-map/mapElement.html', { waitUntil: 'networkidle' });
  });
  ...
  test('Context menu dismissed by Escape key', async () => {
    ...
    const contextMenu = page
      .locator('css= body > gcds-ext-map >> css= div > div.mapml-contextmenu')
      .first();
  ...
  test('Context menu focus on keyboard shortcut', async () => {
    ...
    const resultHandle = await page.evaluateHandle(
      (root: any) => root.activeElement,
      nextHandle
    );
    const nameHandle = await page.evaluateHandle(
      (name: any) => name.outerText,
      resultHandle
    );
  ...
  test('Context menu, back item', async () => {
    ...
    await map.evaluate((map: any) => map.zoomTo(81, -63, 1));
    await page.waitForTimeout(1000);
    await page.getByTestId('firstmap').click({ button: 'right' });
  ...
  test.describe('Context Menu, Toggle Controls ', () => {
    test('Context menu, toggle controls off', async () => {
      ...
      await page.click('body > gcds-ext-map', { button: 'right' });
  ...
  test('Submenu, copy map (MapML)', async () => {
    ...
    const expected = `<gcds-ext-map data-testid="firstmap" style="height: 600px;width:500px;" projection="CBMTILE" zoom="0" lat="47" lon="-92" controls="" role="application">
  ...
  <style>gcds-ext-map > * { display: none!important; }</style></gcds-ext-map>`;
  ...
  test('Submenu, copy location in tilematrix coordinates, which is not implemented, so faked with gcrs', async () => {
    ...
    await page.$eval('body > gcds-ext-map', (map) => {
      ...
      (window as any).M.options.defaultLocCoor = 'tilematrix';
    });
  ...
  test('Context menu, click at margin and move mouse out when submenu is visible', async () => {
    ...
    const contextMenu = await page
      ...
      .first();
    expect(await contextMenu.isVisible()).toBeTruthy();
    const mapSize = await page.$eval('body > gcds-ext-map', (map) => {
  ...
  test('Layer Context menu, Pressing space on checkbox button toggles layer', async () => {
    ...
    const layerCheck = await page.$eval('body > gcds-ext-map', (map) => {
