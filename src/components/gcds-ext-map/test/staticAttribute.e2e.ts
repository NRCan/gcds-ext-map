...
test.describe('Adding Static Attribute to gcds-ext-map', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/test/gcds-ext-map/staticAttribute.html', { waitUntil: 'networkidle' });
    
    // Wait for the gcds-ext-map component to be hydrated and ready
  ...
  test('Setting Static Attribute to gcds-ext-map', async ({ page }) => {
    // Setting static attribute in the gcds-ext-map tag
    await page.$eval('body > gcds-ext-map', (viewer: any) => (viewer.static = true));
    await page.waitForTimeout(250);
    let attribute = await page.$eval('body > gcds-ext-map', (viewer) =>
  ...
  test('Static attribute via HTML attribute', async ({ page }) => {
    ...
    await page.$eval('body > gcds-ext-map', (viewer) => viewer.setAttribute('static', ''));
    
    let hasAttribute = await page.$eval('body > gcds-ext-map', (viewer) =>
      viewer.hasAttribute('static')
    );
    let propertyValue = await page.$eval('body > gcds-ext-map', (viewer: any) => viewer.static);
    ...
    // Remove attribute
    await page.$eval('body > gcds-ext-map', (viewer) => viewer.removeAttribute('static'));
    
    hasAttribute = await page.$eval('body > gcds-ext-map', (viewer) =>
      viewer.hasAttribute('static')
    );
    propertyValue = await page.$eval('body > gcds-ext-map', (viewer: any) => viewer.static);
