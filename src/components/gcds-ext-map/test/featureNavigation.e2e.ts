...
test.describe('Playwright Keyboard Navigation + Query Layer Tests', () => {
  test.describe('Arrow Key Navigable Tests', () => {
    test('Tab focuses inline features', async ({ page }) => {
      await page.goto('/test/gcds-ext-map/featureNavigation.html', { waitUntil: 'networkidle' });
      ...
      const resultHandle = await page.evaluateHandle(
        (root: any) => root.activeElement.querySelector('.leaflet-interactive'),
        nextHandle
      );
      const focused = await (
        await page.evaluateHandle(
          (elem: any) => elem.getAttribute('d'),
          resultHandle
        )
      ).jsonValue();
      ...
      let tooltipCount = await page.$eval(
        'gcds-ext-map .leaflet-tooltip-pane',
    ...
    test('Tab focuses fetched features', async ({ page }) => {
      await page.goto('/test/gcds-ext-map/featureNavigation.html', { waitUntil: 'networkidle' });
      await page.evaluateHandle(() =>
      ...
      const focused = await page.evaluate(
        (root: any) => root.activeElement.getAttribute('aria-label').trim(),
        nextHandle
      );
    ...
    test('When at last feature, arrow down goes to first feature', async ({ page }) => {
      ...
      const activeFeature = await page.$eval('body > gcds-ext-map', (map: any) =>
        map.shadowRoot.activeElement.getAttribute('aria-label')
      );

      await page.keyboard.press('ArrowDown');
      await page.waitForTimeout(500);
      const nextActiveFeature = await page.$eval('body > gcds-ext-map', (map: any) =>
        map.shadowRoot.activeElement.getAttribute('aria-label')
      );
    ...
    test('Center feature is always refocused', async ({ page }) => {
      await page.goto('/test/map-layer/templatedFeaturesFilter.html', { waitUntil: 'networkidle' });
      await page.waitForTimeout(1000);
