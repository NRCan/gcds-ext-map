...
test.describe('Playwright gcds-ext-map Basic Tests', () => {
  let page;
  let context;
  test.beforeAll(async () => {
    ...
    await page.goto('/test/gcds-ext-map/gcds-ext-map.html', { waitUntil: 'networkidle' });
  ...
  test("Ensure attribution control has role='group' aria-label='Map data attribution'", async () => {
    ...
    let role = await page.evaluate(
      `document.querySelector('gcds-ext-map')._map.attributionControl._container.getAttribute('role')`
    );
    expect(role).toEqual('group');
    let arialabel = await page.evaluate(
      `document.querySelector('gcds-ext-map')._map.attributionControl._container.getAttribute('aria-label')`
    );
  ...
  test("Panned and zoomed initial map's extent", async () => {
    await page.$eval('body > gcds-ext-map', (map) => map.zoomTo(81, -63, 1));
    await page.waitForTimeout(1000);
    const extent = await page.$eval('body > gcds-ext-map', (map) => map.extent);
  ...
  test('Reload button takes you back to initial state', async () => {
    ...
    const history = await page.$eval('body > gcds-ext-map', (map) => map._history);
