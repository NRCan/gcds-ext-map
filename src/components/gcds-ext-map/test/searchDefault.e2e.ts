...
test.describe('Search default handler tests', () => {
  ...
  test.beforeAll(async () => {
    ...
    await page.goto('/test/gcds-ext-map/searchDefault.html', { waitUntil: 'networkidle' });
  ...
  test('Clicking a suggestion result triggers search and navigates', async () => {
    ...
    let center = await page.$eval('[data-testid=viewer]', (viewer: any) => {
      let map = viewer._map;
  ...
  test('mapsuggestions event is cancelable', async () => {
    ...
    await page.evaluate(() => {
      let viewer = document.querySelector('[data-testid=viewer]') as any;
    ...
    await page.waitForFunction(
      () =>
        (document.querySelector('[data-testid=viewer]') as any)
          ._suggestionsDefaultPrevented,
      { timeout: 5000 }
    );
    let prevented = await page.$eval(
      '[data-testid=viewer]',
      (viewer: any) => viewer._suggestionsDefaultPrevented
    );
  ...
  test('mapsearch event is cancelable', async () => {
    ...
    await page.waitForFunction(
      () =>
        (document.querySelector('[data-testid=viewer]') as any)._searchDefaultPrevented,
      { timeout: 5000 }
    );
    let prevented = await page.$eval(
      '[data-testid=viewer]',
      (viewer: any) => viewer._searchDefaultPrevented
    );
    expect(prevented).toBe(true);
