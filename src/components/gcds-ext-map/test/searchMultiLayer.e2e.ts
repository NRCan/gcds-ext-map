...
test.describe('Search multi-layer tests', () => {
  ...
  test.beforeAll(async () => {
    ...
    await page.goto('/test/gcds-ext-map/searchMultiLayer.html', { waitUntil: 'networkidle' });
  ...
  test('Suggestion results carry correct layer attribution', async () => {
    let layers = await page.$$eval('.mapml-search-result', (btns) =>
      btns.map((b) => (b as HTMLElement).dataset.layer)
    );
  ...
  test('Event detail responses array has entries from each layer', async () => {
    ...
    await page.evaluate(() => {
      const viewer = document.querySelector('[data-testid=viewer]') as any;
    ...
    await page.waitForFunction(
      () =>
        (document.querySelector('[data-testid=viewer]') as any)._testResponseCount !==
        undefined,
      { timeout: 5000 }
    );
    ...
    let responseCount = await page.$eval(
      '[data-testid=viewer]',
      (v: any) => v._testResponseCount
    );
    let layerLabels = await page.$eval(
      '[data-testid=viewer]',
      (v: any) => v._testLayerLabels
    );
