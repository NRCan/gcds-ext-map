...
test.describe('gcds-ext-map DOM API Tests', () => {
  let page: Page;
  ...
  test.beforeAll(async () => {
    ...
    await page.goto('/test/gcds-ext-map/domApi-gcds-ext-map.html', { waitUntil: 'networkidle' });
  });
  ...
  test('Create a gcds-ext-map with document.createElement(gcds-ext-map)', async () => {
    ...
    const viewerHandle = await page.evaluateHandle(() =>
      document.createElement('gcds-ext-map')
    );
    const nn = await (
      await page.evaluateHandle((viewer) => viewer.nodeName, viewerHandle)
    ).jsonValue();
    expect(nn).toEqual('GCDS-EXT-MAP');
    ...
    const velName = await page.evaluate(
      () => document.body.querySelector('gcds-ext-map')?.nodeName
    );
    expect(velName).toBe('GCDS-EXT-MAP');
    // check for error messages in console
    expect(errorLogs.length).toBe(0);
    await page.waitForTimeout(500);
    // testing to ensure gcds-ext-map was successfully implemented
    let verifymap = await page.evaluate(
      () => document.body.querySelector('gcds-ext-map')?.childElementCount
    );
    ...
    const layerControlHidden = await page.$eval(
      'css=body > gcds-ext-map >> css=div > div.leaflet-control-container > div.leaflet-top.leaflet-right > div',
  ...
  test('Create a layer with document.createElement(map-layer)', async () => {
    ...
    await page.evaluateHandle(
      (layer) => document.querySelector('gcds-ext-map')?.appendChild(layer),
      layerHandle
    );
  ...
  test.skip('Remove gcds-ext-map from DOM, add it back in', async () => {
    // check for error messages in console
    let errorLogs: string[] = [];
    page.on('pageerror', (err) => {
      errorLogs.push(err.message);
    ...
    await viewer.evaluate(() => {
      let m = document.querySelector('gcds-ext-map');
      if (m) {
    ...
    await viewer.evaluate((viewer) => {
      const layer = viewer.querySelector('map-layer') as any;
    ...
    expect(
      await viewer.evaluate(() => {
        let m = document.querySelector('gcds-ext-map');
        let l = m?.querySelector('map-layer') as any;
        return l?.label;
    ...
    const layerTile = await page.locator(
      'body > gcds-ext-map .leaflet-tile-loaded:nth-child(1)'
    );
    expect(
  ...
  test('Toggle all gcds-ext-map controls by adding or removing controls attribute', async () => {
    const viewerHandle = await page.evaluateHandle(() =>
  ...
  test('Create gcds-ext-map without controls', async () => {
    // remove previously created map
    await page.evaluateHandle(() =>
      document.querySelector('gcds-ext-map')?.remove()
    );
  ...
  test('Adding controls to a gcds-ext-map which was created without controls', async () => {
    await page.pause
    const viewerHandle = await page.evaluateHandle(() =>
      document.querySelector('gcds-ext-map')
    );
    await page.evaluate((viewer: any) => (viewer.controls = true), viewerHandle);
    await page.waitForTimeout(500);
  ...
  test('Creating gcds-ext-map with the default size hides the fullscreen control and the controls attribute functions properly', async () => {
    // Adding map
    const viewerHandle = await page.evaluateHandle(() =>
      document.createElement('gcds-ext-map')
    );
  ...
  test.describe('controlslist test', () => {
    test('gcds-ext-map created with controlslist', async () => {
      // Adding map
      const viewerHandle = await page.evaluateHandle(() =>
        document.createElement('gcds-ext-map')
      );
    ...
    test('Adding controlslist using setAttribute and controlsList setter', async () => {
      ...
      // controlsList setter
      await page.evaluate(
        (viewer: any) => (viewer.controlsList = 'noreload nozoom'),
        viewerHandle
      );
    ...
    test('controlslist geolocations test', async () => {
      ...
      //remove geolocation
      await page.evaluate(
        (viewer: any) => (viewer.controlsList = 'noreload'),
        viewerHandle
      );
      hascontrolslist = await page.evaluate(
        (viewer) => viewer?.getAttribute('controlslist'),
    ...
    test('controlslist removeAttribute', async () => {
      ...
      let controlslistDomTokenListLength = await page.evaluate(
        (viewer: any) => viewer.controlsList.length,
        viewerHandle
      );
    ...
    test.describe('DomTokenList Methods test', () => {
      test('using .add(), .replace() and .value() methods of controlsList', async () => {
        ...
        // Testing .value()
        await page.evaluateHandle(
          (viewer: any) => (viewer.controlsList.value = 'nozoom nolayer'),
          viewerHandle
        );
        ...
        let controlsListValue = await page.evaluate(
          (viewer: any) => viewer.controlsList.value,
          viewerHandle
        );
        ...
        // Testing .add()
        await page.evaluateHandle(
          (viewer: any) => viewer.controlsList.add('nofullscreen'),
          viewerHandle
        );
        ...
        // Testing .replace()
        await page.evaluateHandle(
          (viewer: any) => viewer.controlsList.replace('nolayer', 'noreload'),
          viewerHandle
        );
        controlslistAttribute = await page.evaluate(
          (viewer) => viewer?.getAttribute('controlslist'),
        ...
        // Testing .toggle()
        await page.evaluateHandle(
          (viewer: any) => viewer.controlsList.toggle('nofullscreen'),
          viewerHandle
        );
        controlslistAttribute = await page.evaluate(
          (viewer) => viewer?.getAttribute('controlslist'),
      ...
      test('using .contains(), .item(), .remove() and .supports() methods of controlsList', async () => {
        ...
        let controlslistContains = await page.evaluate(
          (viewer: any) => viewer.controlsList.contains('nozoom'),
          viewerHandle
        );
        expect(controlslistContains).toBe(true);
        controlslistContains = await page.evaluate(
          (viewer: any) => viewer.controlsList.contains('nofullscreen'),
          viewerHandle
        );
        ...
        // Testing .remove()
        await page.evaluateHandle(
          (viewer: any) => viewer.controlsList.remove('noreload'),
          viewerHandle
        );
        let controlslistAttribute = await page.evaluate(
          (viewer) => viewer?.getAttribute('controlslist'),
        ...
        let controlslistSupported = await page.evaluate(
          (viewer: any) => viewer.controlsList.supports('noreload'),
          viewerHandle
        );
        expect(controlslistSupported).toBe(true);
        controlslistSupported = await page.evaluate(
          (viewer: any) => viewer.controlsList.supports('nofullscreen'),
          viewerHandle
        );
        expect(controlslistSupported).toBe(true);
        controlslistSupported = await page.evaluate(
          (viewer: any) => viewer.controlsList.supports('nozoom'),
          viewerHandle
        );
        expect(controlslistSupported).toBe(true);
        controlslistSupported = await page.evaluate(
          (viewer: any) => viewer.controlsList.supports('nolayer'),
          viewerHandle
        );
        expect(controlslistSupported).toBe(true);
        controlslistSupported = await page.evaluate(
          (viewer: any) => viewer.controlsList.supports('noscale'),
          viewerHandle
        );
        expect(controlslistSupported).toBe(true);
        // controls which are not yet supported / or invalid
        controlslistSupported = await page.evaluate(
          (viewer: any) => viewer.controlsList.supports('nogeolocation'),
          viewerHandle
        );
        expect(controlslistSupported).toBe(false);
        controlslistSupported = await page.evaluate(
          (viewer: any) => viewer.controlsList.supports('nocode'),
          viewerHandle
        );
        expect(controlslistSupported).toBe(false);
    ...
    test('controls disabled for empty custom projection map', async () => {
      ...
      await page.waitForTimeout(500);
      await page.evaluate((viewer: any) => {
