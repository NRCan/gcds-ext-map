...
test.describe('Layer Context Menu Tests', () => {
  let page;
  let context;
  test.beforeAll(async () => {
    ...
    await page.goto('/test/map-layer/layerContextMenu.html', { waitUntil: 'networkidle' });
  ...
  test('Layer context menu copy layer', async () => {
    ...
    let copyLayer = await mesageLayerArea.inputValue();
    // Remove Stencil hydration attributes
    copyLayer = copyLayer.replace(/ class="hydrated"/g, '');
    expect(copyLayer).toEqual(
      ...
      <map-extent units="CBMTILE" checked="">
        ...
        <map-link rel="tile" tref="http://localhost:3333/test/data/tiles/cbmt/{zoomLevel}/c{col}_r{row}.png"></map-link>
      </map-extent>
    </map-layer>`
    );
  });
  ...
  test('Copy layer with relative src attribute', async () => {
    ...
    expect(copyLayer).toEqual(
      '<map-layer src="http://localhost:3333/test/data/tiles/cbmt/DouglasFir.mapml" label="Natural Resources Canada - Douglas Fir (Genus Pseudotsuga) 250m resolution"></map-layer>'
    );
  });
