...
test.describe('Layer Context Menu Keyboard Tests', () => {
  let page;
  let context;
  test.beforeAll(async () => {
    ...
    await page.goto('/test/map-layer/layerContextMenuKeyboard.html', { waitUntil: 'networkidle' });
  });
  ...
  test('Space bar activates layer context menu item', async () => {
    ...
    const result = await page.evaluate(() => {
      let p = new DOMParser();
      let t = (document.querySelector('[name="messageExtent"]') as HTMLTextAreaElement).value;
      if (t !== '') {
